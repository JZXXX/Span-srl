...
while ($s) {
	...
	if ($pprop and !$gprop) {	    
	    !$options{noW}
#	    and print STDERR "WARNING : sentence $ns : verb ", $pprop->verb,
#	    " at position ", $pprop->position, " : found predicted prop without its gold reference! Skipping prop!\n";
	}
	elsif ($gprop) {
	    if (!$pprop) {
		!$options{noW}
#		and print STDERR "WARNING : sentence $ns : verb ", $gprop->verb,
...
#		$pprop = SRL::prop->new($gprop->verb, $gprop->position);
		...
		!$options{noW}
#		and  print STDERR "WARNING : sentence $ns : props do not match : expecting ",

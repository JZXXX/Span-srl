            ...
            while gold_line:
                ...
                if gold_line.rstrip() != '':
                    ...
                    if gold_node != '_':
                        ...
                        if labeled:
                            gold_edges = set(tuple([gold_edge.split(':', 1)[0],gold_edge.split(':', 1)[1][0]+gold_edge.split(':', 1)[1][gold_edge.split(':', 1)[1].index(',')+1:]]) for gold_edge in gold_node)

                        else:
                            gold_edges = set(tuple([gold_edge.split(':', 1)[0],gold_edge.split(':', 1)[1].split('-',1)[0].split(',')[1].rstrip(')')]) for gold_edge in gold_node)
                    else:
                        gold_edges = set()
                    # if gold_edges:
                    #     pdb.set_trace()
                    ...
                    if sys_node != '_':
                        ...
                        if labeled:
                            ...
                            sys_edges = set(tuple([sys_edge.split(':', 1)[0],sys_edge.split(':',1)[1].split('-',1)[0].lstrip('(').rstrip(')')]) for sys_edge in sys_node)
                    else:
                        sys_edges = set()
                    # if sys_edges:
                    #     pdb.set_trace()

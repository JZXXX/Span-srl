...
class PretrainedVocab(SetVocab):
    ...
    with tf.variable_scope(variable_scope or self.field):
      if self.variable is None:
        with tf.device('/cpu:0'):
          ...
          # ============solve the bug of tensor proto larger than 2GB for glove 840B=================================
          # self.variable = tf.Variable(tf.constant(0.0, shape=self.embeddings.shape),name='Embeddings_weights', trainable=False)
          # self.embed_placeholder = tf.placeholder(tf.float32, self.embeddings.shape, name=self.name + 'Embeddings_ph')
          # embed_weights = self.variable.assign(self.embed_placeholder)
          # ============solve the bug of tensor proto larger than 2GB for glove 840B=======================================
  ...
  def set_placeholders(self, indices, feed_dict={}):
    ...
    # feed_dict[self.embed_placeholder] = self.embeddings
    # pdb.set_trace()
    return feed_dict

...
def get_input_networks(config, network_list, networks):
	if network_list in ('None', ''):
		return set(), networks
	else:
		network_list = network_list.split(':')
		for _network_class in network_list:
			...
			_config = Config(config_file=config_file)
			NetworkClass = getattr(parser, _network_class)
			networks[_network_class] = NetworkClass(config=_config)
		return set(networks[_network_class] for _network_class in network_list), networks
...
def main():
  ...
  train_parser.add_argument('--nornn', action='store_true')
  train_parser.add_argument('--nowarning', action='store_true')
  ...
  run_parser.add_argument('--testing', action='store_true')
  run_parser.add_argument('--debug', action='store_true')
  run_parser.add_argument('--check_iter', action='store_true')
  run_parser.add_argument('--gen_tree', action='store_true')
  run_parser.add_argument('--get_argmax', action='store_true')
  run_parser.add_argument('--nornn', action='store_true')
  for section_name in section_names:
...
def train(**kwargs):
  ...
  nowarning = kwargs.pop('nowarning')
  ...
  if config.get('DEFAULT','auto_dir')=='True':
    save_dir=save_dir+config.get('DEFAULT','modelname')
    kwargs['DEFAULT']['save_dir'] = save_dir
    config = Config(config_file=config_file, **kwargs)
  #pdb.set_trace()
  print (save_dir)
  # If not loading, ask the user if they want to overwrite the directory
  # if not load and os.path.isdir(save_dir) and not nowarning:
  #   #'''
  #   if not force:
  #     input_str = ''
  ...
  input_networks, networks = get_input_networks(config, network_list, {})
  NetworkClass = getattr(parser, network_class)
  network = NetworkClass(input_networks=input_networks, config=config)
  network.train(load=load, noscreen=noscreen, debug=debug,nornn=nornn)
  return
